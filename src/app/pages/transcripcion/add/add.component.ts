...
export class AddTranscriptionComponent implements OnInit {
  ...
  public patientData: any = {
    'diagnostic': {
      'soatInsurance': false,
      'dateStartPatientCondition': '',
      'timeStartPatientCondition': new Date(),
      'laterality': null,
  ...
  public nameComponent: string = 'transcriptionComponent';
  ...
  ngOnInit(): void {
    ...
    this.utilitiesService.fnAuthValidUser().then(response => {
      ...
      let idUser = this.userData['_id'];
      console.log('idUser: ', idUser);
      this.fnGetDataUserById(this.token, { id: idUser }, idUser).then((respDataUser) => {
        console.log('respDataUser: ', respDataUser);
        if(respDataUser['body']['stateRequest']) {
          this.user = respDataUser['body']['data'];
          console.log('this.user: ', this.user);
          this.patientData['dataUser'] = this.user;
          // this.patientData['diagnostic'] = this.user;
  ...
  fnGetDataUserById(token, objectSend, idUser) {
    return new Promise((resolve, reject) => {
      this.userService.fnHttpGetDataUserById(token, objectSend, idUser).subscribe(response => {
        const data = response['body']['data'];
  ...
  fnReturnPage(): void {
    // this.location.back();
    this.utilitiesService.fnNavigateByUrl('pages/transcripcion/lista-transcripciones');
  }
