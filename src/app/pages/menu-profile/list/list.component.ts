...
export class ListComponent implements OnInit {
    ...
    this.fnGetList(token).then((response) => {
      if (response) {
        ...
        collectionData.forEach((value, key) => {
          ...
          let idMenu = value['idMenu'];
          if (idProfile) {
            this.fnGetProfiles(token, idProfile).then((responseState) => {
              if (responseState) {
                value['profileName'] = responseState['body']['data'][0]['name'];
              }
            });
          }
          if (idMenu) {
            this.fnGetMenus(token, idMenu).then((responseState) => {
              if (responseState) {
                console.log('responseState: ', responseState);
                value['menuName'] = responseState['body']['data'][0]['title'];
                value['menuIcon'] = responseState['body']['data'][0]['icon'];
              }
            });
          }
        });
  ...
  fnGetList(token) {
    return new Promise((resolve, reject) => {
      this.menuProfileService.fnHttpGetMenuProfileList(token).subscribe(response => {
        const data = response['body']['data'];
        if (data.length > 0) {
          resolve(response);
        } else {
